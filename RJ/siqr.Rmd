...
est.tau02 <- siqr(y0,X,beta.inital = NULL, tau=0.25)
...
```{r cache=TRUE, eval = FALSE}
est.tau08 <- siqr(y0,X,beta.inital = NULL, tau=0.75)
...
sim.results <- foreach(m = 1:n.sim,.combine = "rbind") %dopar% {
  X <- data$X
  Y <- data$Y[[m]]
  est <- siqr(Y, X, beta.inital = NULL, tau=tau,maxiter = 20,tol = 1e-6)
  est$beta
}
stopCluster(cl)
```
...
```{r}
boxplot(data.frame((sim.results)), outline=T,notch=T,range=1,main = "Boxplots of Coefficient Estimates",horizontal = F)
```
